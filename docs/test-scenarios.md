...
- [ ] `/owner/login`에서 seed 코드 `demo0001`로 로그인한다.
...
> `docs/roadmap-to-launch.md` 2.4 완료 후 검증한다.

- [ ] `/api/llm/draft`가 구현되어 있다.
- [ ] 사장님 편지 작성 화면에 “AI 초안, 꼭 수정해서 보내세요” 맥락이 명확히 표시된다.
- [ ] 명시적 버튼 클릭 시에만 LLM 요청이 발생한다.
- [ ] activity/가게/참여자 맥락이 프롬프트에 반영된다.
- [ ] 초안은 첫 문장 또는 짧은 도입 제안 수준으로 반환된다.
- [ ] 사장님이 수정한 본문만 최종 letter body로 저장된다.
- [ ] `llm_draft`에는 원본 초안이 기록된다.
- [ ] API key가 없거나 LLM 실패 시 수동 작성 플로우가 막히지 않는다.

## 8. `/impact` 노드맵

> `docs/roadmap-to-launch.md` 2.5 완료 후 검증한다. 실데이터 연결을 포기한 경우 fallback 문구를 확인한다.

- [ ] 공개 카드만 노드맵 데이터에 포함된다.
- [ ] 비공개, 신고 처리 removed 카드는 제외된다.
- [ ] project/shop/participant/category 관계가 activity 기준으로 연결된다.
- [ ] hover 또는 tap 시 관련 카드 3~5건을 미리 볼 수 있다.
- [ ] 모바일에서 화면이 깨지지 않는다.
- [ ] 실데이터 연결 전 fallback이면 “시연용 정적 시각화” 문구가 명확하다.
